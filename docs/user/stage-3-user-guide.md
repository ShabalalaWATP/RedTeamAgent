...
Daily AI usage is controlled by `DAILY_REVIEW_RUN_LIMIT`. The server checks this limit before a review run is queued and the review screen shows the user's remaining runs for the current UTC day. Public auth, upload, source-ingestion and review-run endpoints also use rate limits to reduce automated abuse. Account creation and password reset can require either Cloudflare Turnstile or the built-in one-question challenge.
