...
test('stage 1 browser flow reaches evidence-linked report', async ({ page }) => {
  ...
  await page.getByLabel('Upload TXT, Markdown, PDF or DOCX').setInputFiles({
...
test('stage 1 core workflow is keyboard operable', async ({ page }) => {
  await mockApi(page, { initialProjects: [] });
  await page.goto('/auth');
  await expect(page.getByRole('heading', { name: 'RedTeamAgent' })).toBeVisible();
  ...
  await page.keyboard.press('Enter');
  await tabTo(page, page.getByLabel('Upload TXT, Markdown, PDF or DOCX'));
